...
var RightText0 = [];
var LeftText0 = [];
var LK4label = new Array(2);
// Use actual NumberOfScales defined
LK4label ["EN"] = Array.from({length: 3}, e => Array(4).fill(0));
LK4label ["NL"] = Array.from({length: 3}, e => Array(4).fill(0));
var HeaderLineText1 = [];
var RightText1 = [];
var LeftText1 = [];
var HeaderLineText2 = [];
...
language = "EN";
PageTitleText ["EN"] = "Likert 4 point scale Listening Experiment";
...
PageTitleText ["NL"] = "Likert 4 punt schaal Luisterexperiment";
...
for(var l in languageArray) {
	for(var i = 1; i < NumberOfScales; i++) {
		if ( typeof(LK4label [languageArray [l]] [i]) == "undefined" ) {
			LK4label [languageArray [l]] [i] = Array(4).fill(0)
			for(var j = 0; j < 5; j++) {		
				LK4label [languageArray [l]] [i] [j] = LK4label [languageArray [l]] [0] [j];
			};
		};
	};
};
...
function replaceTexts (language) {
	...
	if(VariableText){
		...
		for(var j = 0; j < 4; j++) {
			document.getElementById('LK4.0-'+(j+1)).innerHTML = LK4label [language] [0] [j];
		};
		for(var i = 1; i < NumberOfScales; i++) {
			for(var j = 0; j < 4; j++) {
				if(typeof( LK4label [language] [i]) == "undefined") {
					document.getElementById('LK4.'+ i +'-'+(j+1)).innerHTML = document.getElementById('LK4.'+ (i-1) +'-'+(j+1)).innerHTML;
				} else {
					document.getElementById('LK4.'+ i +'-'+(j+1)).innerHTML = LK4label [language] [i] [j];
				}
			};
		};		
	};
	...
	for(var i = 1; i < NumberOfScales; i++) {
		...
		document.getElementById('LK4row'+i).style.display = '';
