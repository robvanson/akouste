...
Markdown is understood by Large Language Models (LLMs). As an initial step to create a new experiment, it is possible to prompt an LLM to create a markdown file as a first draft. Issuing a suitable prompt with the specifications of the desired experiment, target language, and the descriptions below in the sections [Stimulus related constructs](#STimulus-related-constructs) and [Collecting subject responses](#Collecting-subject-responses), and a suitable markdown example from the [Demonstrations and examples](#Demonstrations-and-examples), can already result in a useful first draft. It is unwise to trust all text and translations of the LLMs on first sight. But the text delivered by an LLM can go a long way towards the desired end result which requires editing to get everything right. An example of a simple prompt text that gives a usefull first draft [can be found here](./Examples/akouste_LLM_simple_prompt.txt).
...
[^7]: The content of the View Stimuli pop-up window cannot be edited in Chrome, Edge, and derived browsers from a local copy of *akousteCreate.html*. That is, in browsers that label the pop-up window as **Not Secure**. The CSV text can be edited, but the edits cannot be saved to the main window. In these cases, the table or CSV text can be copied to, and edited, in a spreadsheet or text file. *Note: Emtpy lines and lines starting with # are removed.*
