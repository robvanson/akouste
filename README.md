...
[^3]: The converter [*markdown-it.min.js*](https://cdn.jsdelivr.net/npm/markdown-it@14.1.0/dist/) can be stored locally if desired. This is done in the Git repository to reduce the dependence on linked outside sources. Be aware that the version in the *akoúste* repository can be outdated.  Besides markdown-it, a second markdown parser is supported, [Remarkable](https://github.com/jonschlinkert/remarkable). Remarkable wil be used when the local file [*remarkable.min.js*](https://app.unpkg.com/remarkable@2.0.1/files/dist) is present, or when the URL parameter **?MarkdownParser=Remarkable** is given when loading *acousteCreate.html*. The Remarkable version used does not support the *linkify* option.
